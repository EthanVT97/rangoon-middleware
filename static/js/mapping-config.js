...
function initializeForm() {
    ...
    document.getElementById('mappingForm').addEventListener('submit', handleFormSubmit);
}
...
function populateFormWithTemplate(template) {
    ...
    document.getElementById('mappingName').value = template.mapping_name;
    document.getElementById('mappingDescription').value = template.description || '';
    document.getElementById('erpEndpoint').value = template.erp_endpoint;
    ...
    template.source_columns.forEach((col, index) => {
        ...
        row.querySelector('.column-name').value = col.name;
        row.querySelector('.column-type').value = col.type;
        row.querySelector('.column-required').checked = col.required || false;
    });
    ...
    Object.entries(template.target_columns).forEach(([fieldName, mapping], index) => {
        ...
        row.querySelector('.target-field').value = fieldName;
        if (mapping.source_column) {
            row.querySelector('.source-mapping').value = mapping.source_column;
        }
        if (mapping.transformation) {
            row.querySelector('.transformation').value = mapping.transformation;
        }
        if (mapping.default_value) {
            row.querySelector('.default-value').value = mapping.default_value;
        }
        row.querySelector('.field-required').checked = mapping.required || false;
    });
...
function clearForm() {
    ...
    // Clear first rows
    if (sourceRows[0]) {
        sourceRows[0].querySelector('.column-name').value = '';
        sourceRows[0].querySelector('.column-type').value = 'string';
        sourceRows[0].querySelector('.column-required').checked = false;
    }
    
    if (targetRows[0]) {
        targetRows[0].querySelector('.target-field').value = '';
        targetRows[0].querySelector('.source-mapping').value = '';
        targetRows[0].querySelector('.transformation').value = '';
        targetRows[0].querySelector('.default-value').value = '';
        targetRows[0].querySelector('.field-required').checked = false;
    }
